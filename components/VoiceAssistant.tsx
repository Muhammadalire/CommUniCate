...
export default function VoiceAssistant() {
  ...
  const getSystemInstruction = () => {
    ...
    return base + topic + "Keep your responses relatively brief to encourage a back-and-forth dialogue. Do not sound robotic or overly formal.";
  };
  ...
  const connect = async () => {
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            ...
            // Send an initial text message to prompt the AI to speak first
            sessionPromise.then((session) => {
              session.sendRealtimeInput({
                text: 'Hi! Please introduce yourself and start the conversation based on your persona and our chosen topic.'
              });
            });
            
            // Start capturing and sending audio
            processorRef.current = createAudioProcessor(audioCtx, stream, (base64Data) => {
              sessionPromise.then((session) => {
                session.sendRealtimeInput({
          ...
          onmessage: (message: LiveServerMessage) => {
            ...
            // Handle interruptions (user started speaking)
      ...
      setErrorMessage(err.message || "Failed to access microphone or connect to AI.");
      disconnect();
    }
  };
    ...
    <div className="min-h-screen bg-[#FAF7F2] flex flex-col items-center justify-between p-4 md:p-8 relative overflow-hidden font-sans text-stone-800 selection:bg-orange-200">
      ...
      <div className="w-full flex justify-center z-10 mt-auto pt-8">
        <div className="flex items-start gap-3 max-w-2xl bg-white/40 backdrop-blur-md p-4 rounded-2xl border border-white/60 text-xs text-stone-500 leading-relaxed shadow-sm">
          ...
          <p>
            <strong>Architecture Note:</strong> To provide an instant, zero-setup experience, this app uses the <strong>Gemini Live Audio API</strong> for real-time voice. Implementing LiveKit + Groq + Fish Audio requires deploying a custom Node.js WebRTC server and providing three separate API keys. The UI and UX reflect the requested design!
          </p>
        </div>
      </div>
    </div>
